...
\def\collegeZh {科技學院}
\def\collegeEn {College of Science and Technology}
\def\deptshort {AI}
\def\deptZh    {人工智慧與機器人碩士學位學程}%XX學系XXX碩士班(請參考「學位論文撰寫體例參考」附錄)
\def\deptEn    {Master's Degree Program in Artificial Intelligence and Robotics}
...
\def\logo      {logo-NCNU.eps}   % logo on titlepage. Watermark see below. 校徽在封面。若你想加，樣板已附上，填入 ``logo-NCNU.eps'' 即可
\def\authorZh  {君の名は。}
...
\def\author    {君の名は。}
\def\degreedateROC{中~華~民~國~一百一十四~年~六~月}
\def\degreeyearEn {2025}
\def\degreeyearROC{114} % year of ROC
\def\degreemonth  {06} % MM , use 06 not 6
\def\copyyear  {2025-2027}
